...
class ApiRequestor
{
    ...
    private static function _telemetryJson($requestTelemetry)
    {
        ...
        if ($result != false) {
            ...
            Stripe::getLogger()->error("Error al serializar la carga útil de telemetría.");
            return "{}";
        }
    }
    ...
    public function handleErrorResponse($rbody, $rcode, $rheaders, $resp)
    {
        if (!is_array($resp) || !isset($resp['error'])) {
            $msg = "Objeto de respuesta no válido de API: $rbody "
    ...
    private function _processResourceParam($resource, $hasCurlFile)
    {
        if (get_resource_type($resource) !== 'stream') {
            throw new Error\Api(
                'Intentó cargar un recurso que no es una transmisión.'
            );
        }
        ...
        if ($metaData['wrapper_type'] !== 'plainfile') {
            throw new Error\Api(
                'Solo se admiten flujos de recursos de archivo plano'
            );
        }
    ...
    private function _interpretResponse($rbody, $rcode, $rheaders)
    {
        ...
        if ($resp === null && $jsonError !== JSON_ERROR_NONE) {
            $msg = "Cuerpo de respuesta no válido de API:$rbody "
