    ...
    public static function verifyHeader($payload, $header, $secret, $tolerance = null)
    {
        ...
        if ($timestamp == -1) {
            throw new Error\SignatureVerification(
                "No se puede extraer la marca de tiempo y las firmas del encabezado",
                $header,
                $payload
            );
        }
        if (empty($signatures)) {
            throw new Error\SignatureVerification(
                "No se encontraron firmas con el esquema esperado",
                $header,
                $payload
            );
        }
        ...
        if (!$signatureFound) {
            throw new Error\SignatureVerification(
                "No se encontraron firmas que coincidan con la firma esperada para la carga útil",
                $header,
                $payload
            );
        }
        ...
        if (($tolerance > 0) && (abs(time() - $timestamp) > $tolerance)) {
            throw new Error\SignatureVerification(
                "Marca de tiempo fuera de la zona de tolerancia",
                $header,
                $payload
            );
        }
