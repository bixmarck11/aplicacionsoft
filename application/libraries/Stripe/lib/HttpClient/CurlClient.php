...
class CurlClient implements ClientInterface
{
    ...
    public function request($method, $absUrl, $headers, $params, $hasFile)
    {
        ...
        if (is_callable($this->defaultOptions)) { // call defaultOptions callback, set options to return value
            ...
            if (!is_array($opts)) {
                throw new Error\Api("Valor que no es de matriz devuelto por defecto Opciones de devolución de llamada de CurlClient");
            }
        ...
        if ($method == 'get') {
            if ($hasFile) {
                throw new Error\Api(
                    "Emitir una solicitud GET con un parámetro de archivo"
                );
            }
